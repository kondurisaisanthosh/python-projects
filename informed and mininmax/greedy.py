...
def greedySearch(graph, begin, dest):              ###called method which returns shortest path
    visitedvertex= set()
    expandednode=[]
        ...
        if node not in visitedvertex:
            ...
            for i in graph[node]:
                if(i!='heur' and i not in visitedvertex):
                    ...
                    path_q.put((cost, alternate_path))
...
Greedypath,states= greedySearch(graph, 'S', 'G')
print("Greedy expanded nodes =","->".join(states))
print("Greedy short path =","->".join(Greedypath))